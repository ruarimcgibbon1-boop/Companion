...
describe('setup detectors', () => {
  ...
  it('produces a confirmed breakout when price closes above resistance on volume', () => {
    ...
    const setups = detectSetups({ symbol: 'BRK', price: 5.11, candles, sessionLevels: sl, technical, levels, catalystScore: 10, hasCatalyst: true, spreadPct: 0.1, changePct: 8, session: 'regular', minutesSinceOpen: 60, float: 5_000_000 })
  ...
  it('detects a VWAP reclaim when price closes back above VWAP', () => {
    ...
    const setups = detectSetups({ symbol: 'VW', price: 4.96, candles, sessionLevels: sl, technical, levels, catalystScore: 8, hasCatalyst: true, spreadPct: 0.15, changePct: 4, session: 'regular', minutesSinceOpen: 60, float: 5_000_000 })
